...
![](../.gitbook/assets/secure_forward%20%281%29.png)

The `out_secure_forward` output plugin sends messages via **SSL with authentication** \(cf. [in\_secure\_forward](../input/secure_forward.md)\). This document doesn't describe all parameters. If you want to know full features, check the Further Reading section.

## Installation

`out_secure_forward` is **not** included in either `td-agent` package or `fluentd` gem. In order to install it, please refer to the [Plugin Management](../deployment/plugin-management.md) article.
