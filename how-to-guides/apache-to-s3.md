...
* goal
  * how to use Fluentd's Amazon S3 Output plugin ([`out_s3`](../output/s3.md)\) /
    * aggregate semi-structured logs | realtime

## Background

* log aggregation's goal
  * data archiving
...
* Fluentd does 3 things
  1. CONTINUOUSLY "tails" the access log file
  2. the incoming log entries are 
     1. parsed -- into -- meaningful fields
     2. buffer them
  3. it writes the buffered data -- PERIODICALLY to -- Amazon S3
...
Let's start configuring Fluentd
* If you used the deb/rpm package, Fluentd's config file is located at `/etc/fluent/fluentd.conf`.
...
Please make sure that your Apache outputs are in the default **combined** format
* `format apache2` cannot parse custom log formats
* Please see the [`in_tail`](../input/tail.md) article for more information.
...
The output destination will be Amazon S3
...
* If a matching tag is found in a log, then the config inside `<match>...</match>` is used \(i.e. the log is routed according to the config inside\)
* In this example, the `s3.apache.access` tag \(generated by `tail`\) is always used.

## Test

To test the configuration, just ping the Apache server
...
WARNING: By default, files are created on an hourly basis \(around xx:10\)
* This means that when you first import records using the plugin, no file is created immediately
* The file will be created when the `timekey` condition has been met
* To change the output frequency, please modify the `timekey` value
...
* [Fluentd](http://www.fluentd.org/) is an open-source project under [Cloud Native Computing Foundation \(CNCF\)](https://cncf.io/)
